...
def hexDump(bytes):
    ...
    if (len(bytes) % 8 != 0):
        print
        string.rjust("", 11), repr(bytes[i - 7:i + 1])
...
class OSCBundle:
    ...
    def append(self, address, *msg):
        if isinstance(address, str):
            ...
            raise Exception(
                'invalid type of first argument to OSCBundle.append(), need address string or OSCMessage, not ',
                str(type(address)))
...
def readInt(data):
    if (len(data) < 4):
        print
        "Error: too few bytes for int", data, len(data)
        rest = data
        integer = 0
    else:
...
def readFloat(data):
    if (len(data) < 4):
        print
        "Error: too few bytes for float", data, len(data)
        rest = data
        float = 0
    else:
...
if __name__ == "__main__":
    ...
    print
    decodeOSC(bundlebinary)

    print
    "Testing the callback manager."
    ...
    print
    "sending a bundle to the callback manager"
